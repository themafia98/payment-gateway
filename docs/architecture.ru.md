...
Их и держат порознь. Первое живёт в `@pg/core` и написано один раз. Второе — в плагинах, по
...
**`@pg/runtime-browser`** — то, для чего нужен браузер: раннеры, которые сабмитят формы в
...
**`@pg/ui`** — визуальный кит. Обычный CSS, темизация через `--pg-*`.

**`@pg/provider-*`** — по одному на интеграцию. Протокол живёт здесь и только здесь.
