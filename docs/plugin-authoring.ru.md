...
Инструмент бывает пяти видов, и `capabilities.instruments` должен перечислять ровно те, что вы
принимаете: по нему чекаут строит форму, и контрактный набор проверяет обе стороны.

| Вид              | Что несёт                  | Кто собирает              |
| ---------------- | -------------------------- | ------------------------- |
| `card`           | номер, срок, код           | собственная форма чекаута |
| `token`          | id сохранённой ранее карты | ваш API, в прошлый визит  |
| `hosted_session` | id начатой вами сессии     | ваш API                   |
| `wallet`         | payload из SDK кошелька    | кошелёк, если он у хоста  |
| `none`           | ничего                     | совсем другое место       |

Вид, который вы не принимаете, отклоняйте с кодом `unsupported_instrument`. Это единственный
код ошибки, который читают движок и контрактный набор: любой другой означает «не подошёл
конкретный инструмент», а это другое.

Сохранённую карту стоит назвать отдельно — на ней легко ошибиться: это `token`, в браузере
лежат id и последние четыре цифры, а дальше платёж идёт ровно как с набранной картой — его
могут отклонить, и банк по-прежнему может попросить аутентификацию.
