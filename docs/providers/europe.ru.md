...
там разобраны пять видов действий и правило, что у плагина не бывает секретного ключа.
...
- **host-to-host**: карту отправляете вы, в ответ получаете ACS-URL для 3-D Secure. Это та
  форма, которой следует `@checkout-kit/provider-acquiring`.
...
- **BLIK** в Польше умеет ещё и шестизначный код, который покупатель вводит на вашей
  странице. Дальше он подтверждает в банковском приложении, а вы ждёте. Это действие
  `display` с `format: 'code'` — см.
  [«Показать код и ждать»](./asia.ru.md#показать-код-и-ждать).
...
запросе и числовыми статусами. `@checkout-kit/provider-acquiring` написан именно под эту форму:
