...
Шесть плагинов в этом репозитории списаны с реальных интеграций. Здесь — сами реальные:
...
Это единственное место, где всё устроено не как в проде, — что писать на той стороне, см.
[Бэкенд, с которым говорит плагин](./backend.ru.md).
...
4. Даёт код, который покупатель оплачивает в другом месте? → `display`
5. От покупателя ничего не нужно, нужно просто время? → `poll`

Если ничто не подошло — это повод обсудить, а не сразу добавлять шестой вид. Все перечисленные
здесь интеграции укладываются в пять.
