...
алфавитом UTF-8». Она говорит браузеру, *что за штука* к нему пришла и *каким набором
символов* её читать.
...
подсовывает символы в *другой* кодировке, чтобы проскочить мимо фильтров (XSS через
...
**Чем отличается от похожих.** `Content-Type` говорит, *что это за файл*. Следующий
...
**Простыми словами.** «Показывать меня в своей рамке может *только этот* сайт».
...
другом месте. С `frame-ancestors` любой *чужой* сайт, попытавшийся вставить страницу в
...
**Простыми словами.** «По умолчанию странице нельзя грузить *ничего* — ни скриптов, ни
...
**Почему не строже?** Слово `'unsafe-inline'` звучит страшно, и для *скриптов* это
было бы опасно. Для *стилей* риск маленький. Более чистая альтернатива — вынести CSS в
...
несёт этот пароль. Браузер запускает *только* те скрипты, что показывают подходящий
...
**Альтернативы.**
- **Hash (хэш)** — вместо случайного пароля разрешить скрипт, чьё *содержимое*
...
**Простыми словами.** «Любая форма на этой странице может отправляться только *к нам*».
...
который *притворяется* безобидной картинкой, но на деле содержит скрипт. Без `nosniff`
...
**Чем отличается от `Content-Type`.** `Content-Type` *заявляет* тип; `nosniff`
*запрещает перепроверять* это заявление. Один объявляет — другой обеспечивает.
...
браузер обычно шепчет получателю: «кстати, я пришёл с *этого* URL». Это **referrer**.
...
- `strict-origin` — шлёт только *имя сайта* (например, `https://bank.com`), никогда не
...
*какую информацию мы раскрываем, когда сами обращаемся наружу*; COOP/COEP/CORP
управляют тем, *как другие страницы могут взаимодействовать с нами*. Разное направление
защиты.
...
**Простыми словами.** «Всё, что я гружу со стороны, должно *явно согласиться* быть
...
**Простыми словами.** «Я разрешаю *другому* origin (а именно нашему приложению чекаута)
...
от встраивания чужаками. Но *весь наш смысл* — быть встроенными нашим же приложением,
...
| **CORP** | Кому разрешено встраивать *меня*? | Дать нашему приложению встроить страницу через origin. |
...
- **`SameSite=None`** — вот это хитрое. По умолчанию браузеры *отказываются* слать
