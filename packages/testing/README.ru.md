...
и регистрирует worker в `apps/demo/src/app/enable-mocking.ts`. При обычном `npm run dev` или любой
...
и шлёт её в заголовке `Idempotency-Key`. Это как приклеить к запросу бирку: _«это
попытка #abc123»_. Сервер запоминает эту бирку. Если приходит запрос с уже виденной биркой,
...
packages/testing/src/backend/
  ...
  config.ts           границы задержки, OTP (тестовые карты — в packages/testing)
...
- **Новая тестовая карта:** добавь запись в `TEST_CARDS` в `packages/testing/src/test-cards.ts`.
  Handler confirm берёт поведение оттуда — больше ничего менять не нужно.
